...
## 기능 구현하면서 느꼈던 점
아직 자바에 익숙하지 않은 것 같다. <br/>
그리고 MVC로 구현을 했는데 아직 어떻게 돌아가는지 정확하게 파악이 덜된 느낌이다.<br/>
MVC로 처음해보아서 그런지 이게 맞는 건지 잘 모르겠다. <br/>
하지만 다른 사이트들을 보면서 공부를 했다.<br/>
Random으로 숫자를 어떻게 뽑을지에 고민을 많이 했다.<br/> 
열심히 생각해봤는데 내 수준에서는 저것이 지금의 최선이다. <br/>
하지만 더욱 공부를 해서 Random부분을 수정할 것이다.<br/>
이번에 리스트에 다시 알게 되었고 boolean을 사용해서 최대한 코드를 짧게 만드는 방법을 구현해 보았다.<br/> 
아직 코드가 많이 긴 것 같다.<br/>
다른 메서드들을 알았으면 코드를 더 줄일 수 있지 않을까 싶다.<br/> 
다음에는 equals 메서드를 사용하면 더 좋을 것 같다. <br/>